...
argparser.add_argument('N', type=int, help="defines the index for the record to examine")
argparser.add_argument('-s','--study', type=int, default=None, help="the study you want to get processed")
argparser.add_argument('-d','--drop_rate', type=float, default=0.9, help="the drop rate used to remove noise")
argparser.add_argument('-md','--model_dir', default=None, help="defines the directory with the checkpoint of the model weights")
...
if args.study is not None:
    ...
    dataset = MimicCxrDataset(dataset_path, dataframe)

if len(dataset) < 1:
    print("no record with that study id\n")
    exit(1)

values = dataset[args.N]
text = values["texts"][0]
...
def summarize_attributions(attributions, dim=-1):
...
def drop_and_normalize(x, discard_ratio=0):
...
mask = drop_and_normalize(mask, args.drop_rate)
