...
label_dataframe = pd.read_csv("./MIMIC-CXR/mimic-cxr-2.0.0-negbio.csv")
label_dataframe = label_dataframe.loc[label_dataframe['study_id'].isin(dataframe['study_id'])]
...
class MimicCxrDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if idx >= len(self.dataframe):
            ...
            labels_to_check = label_dataframe.loc[label_dataframe['study_id'] == text_row.study_id] == 1.0
            searching = True
            while searching:
                randomIdx = random.randint(0, len(self.dataframe) - 1)
                image_row = self.dataframe.iloc[randomIdx]
                image_labels = label_dataframe.loc[label_dataframe["study_id"] == image_row.study_id]
                searching = text_row.study_id == image_row.study_id or (image_labels[labels_to_check] == 1.0).any
        else:
